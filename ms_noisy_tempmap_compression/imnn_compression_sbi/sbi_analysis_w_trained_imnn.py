...
"""Parallel SNPE pipeline for Temp+Noise+IMNN summaries.

This version keeps SNPE training sequential (as required) and parallelizes the
expensive MUSIC+map simulation stage within each round.
"""
...
TARGET_MAP = (
    f"{PROJECT_DIR}/target_data/target_tempmap_w_noise_ms_seed1259935638.npy"
)

MODEL_DIR = "/scratch/subhankar/thesis/codes/imnn_w_ms_temp_map_w_noise/imnn_output_opt_v2_batch_128_lr_5e-5_reg_1e-2_correct/model_opt_v2_batch_128_lr_5e-5_reg_1e-2"
...
def simulate_temp_map_worker(qhii_val, log10mmin_val, seed):
    ...
    music_workdir = os.path.join(outpath, "music_workdir")
    original_cwd = os.getcwd()

    os.makedirs(outpath, exist_ok=True)
    os.makedirs(music_workdir, exist_ok=True)

    try:
        os.chdir(music_workdir)
        two_lpt.run_music(
            cfg["music_exec"],
            cfg["box"],
            [cfg["z_target"]],
...
def worker_simulation(task):
    ...
    try:
        ...
        return {
            "idx": idx,
            "tb_ms": tb_sim.astype(np.float32, copy=False),
            "sim_seed": sim_seed,
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PARALLEL SBI PIPELINE STARTED")
    logger.info("=" * 80)
    ...
    logger.info("Target seed: %d", TARGET_SEED)
    logger.info("Target map path: %s", TARGET_MAP)
    logger.info("Model directory: %s", MODEL_DIR)
    logger.info("Noise directory: %s", NOISE_DIR)
    ...
    logger.info("Target dtype: %s", tb_obs.dtype)
    logger.info("Target mean: %.6e", np.mean(tb_obs))
    logger.info("Target std: %.6e", np.std(tb_obs))
    ...
    logger.info("Estimator path: %s", estimator_path)
    ...
    with np.load(estimator_path) as est_data:
        logger.info("Estimator keys: %s", list(est_data.files))
    ...
    logger.info(
        "Estimator shapes: Finv=%s theta_fid=%s dmu_dtheta=%s Cinv=%s mu=%s",
        finv_arr.shape,
        theta_fid_arr.shape,
        dmu_dtheta_arr.shape,
        cinv_arr.shape,
        mu_arr.shape,
    )
    ...
    try:
        for round_idx in range(args.rounds):
            ...
            for result in tqdm(
                ...
                idx = result["idx"]
                tb_ms = result["tb_ms"]
                summaries[idx] = get_summary(tb_ms[..., None])
