...
"""Compute cylindrical 2D power spectra from mean-subtracted noisy brightness-temperature maps and build covariance.

This script is designed for HPC runs.

Inputs
------
- Training mean-subtracted noisy Tb map dictionary (.npy): seed -> 3D cube (mK)
- Validation mean-subtracted noisy Tb map dictionary (.npy): seed -> 3D cube (mK)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compute cylindrical 2DPS (linear k-bins) from mean-subtracted noisy Tb maps, "
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "temp_plus_noise_maps_ms/temp_plus_noise_maps_ms_training/"
            "training_temp_plus_noise_ms_main.npy"
        ),
        help="Path to training mean-subtracted noisy Tb map dictionary (.npy).",
    )
    parser.add_argument(
        "--val-input",
        default=(
            "/scratch/subhankar/thesis/codes/imnn_w_ms_temp_map_w_noise/"
            "temp_plus_noise_maps_ms/temp_plus_noise_maps_ms_validation/"
            "validation_temp_plus_noise_ms_main.npy"
        ),
        help="Path to validation mean-subtracted noisy Tb map dictionary (.npy).",
    )
    parser.add_argument(
        "--outdir",
        default=(
            "/scratch/subhankar/thesis/codes/2dps_paramter_estimation/noisy_Tb/cov_files"
        ),
        help="Directory to store outputs (2DPS, covariance, plots, logs).",
    )
    parser.add_argument(
        "--logfile",
        default="temp_plus_noise_ms_to_2dps_and_covariance_script_linear.log",
...
def plot_first5_train_val_2dps(
    *,
    ...
    fig.suptitle(
        "First 5 cylindrical 2D power spectra (training: top, validation: bottom)",
        fontsize=13,
        fontweight="bold",
        y=0.98,
    )
...
def main() -> None:
    ...
    logger.info("=" * 90)
    logger.info("NOISY MS TB MAPS -> CYLINDRICAL 2DPS -> COV/CORR (SCRIPT, LINEAR) STARTED")
    logger.info("=" * 90)
    ...
    # Determine shape from first training map
    ...
    train_2dps_out = os.path.join(args.outdir, "2dps_training_temp_plus_noise_ms_main.npy")
    val_2dps_out = os.path.join(args.outdir, "2dps_validation_temp_plus_noise_ms_main.npy")

    first5_png = os.path.join(args.outdir, "first5_training_validation_2dps_temp_plus_noise_ms.png")
    first5_pdf = os.path.join(args.outdir, "first5_training_validation_2dps_temp_plus_noise_ms.pdf")

    cov_out = os.path.join(args.outdir, "covariance_training_plus_validation_2dps_temp_plus_noise_ms.npy")
    corr_out = os.path.join(args.outdir, "correlation_training_plus_validation_2dps_temp_plus_noise_ms.npy")

    covcorr_png = os.path.join(
        args.outdir,
        "covariance_correlation_training_plus_validation_2dps_temp_plus_noise_ms.png",
    )
    covcorr_pdf = os.path.join(
        args.outdir,
        "covariance_correlation_training_plus_validation_2dps_temp_plus_noise_ms.pdf",
    )

    np.save(train_2dps_out, train_ps_dict)
    np.save(val_2dps_out, val_ps_dict)

    cbar_label = r"$P(k_\parallel, k_\perp)$ [mK$^2$]"
