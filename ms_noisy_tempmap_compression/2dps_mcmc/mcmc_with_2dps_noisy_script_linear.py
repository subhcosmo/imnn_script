...
"""MCMC with noisy cylindrical 2D power spectrum (10x10; no mean-noise subtraction).

This script runs emcee MCMC using the *noisy* cylindrical 2D power spectrum as the
summary statistic.
...
5) Compute 10x10 linearly binned cylindrical 2DPS using the `script` backend
6) Compare to the precomputed observed noisy 2DPS with a Gaussian likelihood
...
- Script is intended to run on the cluster from:
  /scratch/subhankar/thesis/codes/2dps_paramter_estimation/noisy_Tb
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="MCMC with noisy cylindrical 2DPS (10x10)")
...
# Observed noisy 2DPS (mock observation; already mean-subtracted noisy cube)
OBS_2DPS_FILE = (
...
COV_MATRIX_FILE = f"{PROJECT_DIR}/cov_files/covariance_training_plus_validation_2dps_temp_plus_noise_ms.npy"
...
logger = setup_logging(os.path.join(args.outdir, "mcmc_2dps_noisy_run.log"))

logger.info("=" * 80)
logger.info("MCMC WITH NOISY CYLINDRICAL 2DPS (NO MEAN-NOISY SUBTRACTION) - START")
logger.info("=" * 80)
...
logger.info("Loaded observed noisy 2DPS: %s", OBS_2DPS_FILE)
...
if ps2d_obs.shape != expected_shape:
    logger.error("Observed 2DPS shape mismatch: got %s, expected %s", ps2d_obs.shape, expected_shape)
    sys.exit(1)
...
def compute_flat_2dps_from_Tb(Tb_ms: np.ndarray) -> np.ndarray:
    """Compute flattened 10x10 cylindrical 2DPS from mean-subtracted noisy Tb cube."""

    ps2d, kount = ionization_map.get_binned_powspec_cylindrical(
        Tb_ms.astype(np.float32, copy=False),
        k_par_edges,
        k_perp_edges,
        convolve=False,
        units="",
...
def simulate_noisy_2dps(QHII_val: float, log10Mmin_val: float) -> np.ndarray:
    """Simulate noisy flattened cylindrical 2DPS for given parameters (fixed noise)."""
...
def log_likelihood(params: np.ndarray) -> float:
    """Gaussian log-likelihood with noisy cylindrical 2DPS."""
    QHII, log10Mmin = params

    try:
        ps_theta = simulate_noisy_2dps(QHII, log10Mmin)
    except Exception:
        return -np.inf
...
for i in range(nwalkers):
    while True:
        ...
        if (QHII_min < proposal[0] < QHII_max) and (log10Mmin_min < proposal[1] < log10Mmin_max):
            p0[i] = proposal
            break
...
chain_file = os.path.join(args.outdir, "mcmc_2dps_noisy_chain.npy")
...
logger.info("")
logger.info("=" * 60)
logger.info("MCMC RESULTS (Noisy Cylindrical 2DPS Summary)")
logger.info("=" * 60)
...
results_file = os.path.join(args.outdir, "mcmc_2dps_noisy_results.npz")
np.savez(
    results_file,
    chain=mcmc_chain,
...
handles = [
    plt.Line2D([], [], color="C1", label="MCMC Posterior (Noisy 2DPS)"),
    plt.Line2D([], [], color="red", linestyle="--", label="Truth"),
]
fig.legend(handles=handles, loc="upper right", fontsize=12, frameon=False)

corner_file = os.path.join(args.outdir, "mcmc_2dps_noisy_corner.png")
...
trace_file = os.path.join(args.outdir, "mcmc_2dps_noisy_trace.png")
...
marginal_file = os.path.join(args.outdir, "mcmc_2dps_noisy_marginals.png")
...
logger.info("")
logger.info("=" * 80)
logger.info("MCMC WITH NOISY CYLINDRICAL 2DPS COMPLETED")
logger.info("=" * 80)
