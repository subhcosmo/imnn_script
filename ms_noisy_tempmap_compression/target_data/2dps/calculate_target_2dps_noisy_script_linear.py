...
"""Compute noisy-target cylindrical 2DPS and plot the full map-processing pipeline.

This script assumes these cubes are already prepared and available:
1. Raw temperature map.
2. Noise map.
3. Noise-added temperature map.
4. Mean-subtracted noisy temperature map.

It computes the 10x10 linearly binned cylindrical 2D power spectrum from the
mean-subtracted noisy cube using the `script` backend and creates a 2x6
diagnostic figure with row-wise operators.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compute 10x10 linear cylindrical 2DPS from a prepared mean-subtracted "
            "noisy Tb cube and generate a 2x6 diagnostic plot."
        )
    )
    parser.add_argument(
        "--raw_map",
    ...
    parser.add_argument(
        "--output_prefix",
        default="target_tempmap_w_noise_ms_seed1259935638",
        help="Prefix for output filenames.",
    )
    parser.add_argument(
        "--logfile",
        default="calculate_target_2dps_noisy_script_linear.log",
...
def add_row_annotations(fig: plt.Figure, row_axes: np.ndarray) -> None:
    ax0, ax1, ax2, ax3, ax4, ax5 = row_axes
    p0 = ax0.get_position()
    p1 = ax1.get_position()
    p2 = ax2.get_position()
    p3 = ax3.get_position()
...
def make_overview_plot(
    *,
    ...
    fig, axes = plt.subplots(2, 6, figsize=(31.0, 10.0))
    ...
    for row_index, (
        ...
        if ps_norm is not None:
            ...
            im_ps = ax_ps.imshow(ps2d, origin="lower", cmap="viridis", extent=extent_ps, aspect="auto")
    ...
    fig.suptitle("Noisy Tb pipeline -> 2DPS (script, linear bins)", fontsize=15, fontweight="bold")
...
def main() -> None:
    ...
    logger.info("=" * 80)
    logger.info("NOISY TARGET TB -> 2DPS (SCRIPT, LINEAR) STARTED")
    logger.info("=" * 80)
    ...
    for path in (args.raw_map, args.noise_map_npz, args.noisy_map, args.mean_subtracted_noisy_map):
    ...
    if not (raw_cube.shape == noise_cube.shape == noisy_cube.shape == ms_noisy_cube.shape):
        raise ValueError(
    ...
    logger.info("Mean(mean-subtracted noisy map): %.8e mK", float(np.mean(ms_noisy_cube, dtype=np.float64)))
    ...
    logger.info("=" * 80)
    logger.info("NOISY TARGET TB -> 2DPS (SCRIPT, LINEAR) COMPLETE")
    logger.info("=" * 80)
