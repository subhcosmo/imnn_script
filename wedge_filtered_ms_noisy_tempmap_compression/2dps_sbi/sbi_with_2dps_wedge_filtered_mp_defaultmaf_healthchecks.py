...
"""Parallel SNPE pipeline for wedge-filtered noisy 2DPS summaries with health checks.
...
4) Applies a foreground-wedge cut in 3D Fourier space and inverse FFTs back.
5) Computes a 10x10 linear-binned cylindrical 2DPS using script backend.
6) Trains SNPE (default MAF) round-by-round and reports health diagnostics.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Parallel SBI with default MAF for wedge-filtered noisy cylindrical "
            "2DPS summaries and round-wise health checks."
        )
    )
    parser.add_argument(
        "--outdir",
...
def get_3d_k_grids(ngrid: int, box: float) -> tuple[np.ndarray, np.ndarray]:
...
def worker_simulation(task):
    ...
    try:
        ...
        tb_wedge_filtered = apply_wedge_filter(tb_noisy_ms, _WORKER_CFG["wedge_mask"])
        summary_flat, neg_bin_frac = compute_2dps_summary(tb_wedge_filtered)
...
def main() -> None:
    ...
    logger.info("=" * 80)
    logger.info("PARALLEL WEDGE-FILTERED NOISY 2DPS SBI PIPELINE STARTED")
    logger.info("=" * 80)
    ...
    logger.info(
        "Summary mode: 2DPS of wedge-filtered mean-subtracted simulated Tb plus noise; "
        "no noise-mean 2DPS subtraction."
    )
    logger.info("Wedge slope C: %.6f", args.wedge_C)

    k_par_grid, k_perp_grid = get_3d_k_grids(NGRID, BOX)
    wedge_mask = k_par_grid >= args.wedge_C * k_perp_grid
    logger.info(
    ...
    samples_path = os.path.join(args.outdir, "snpe_2dps_wedge_filtered_posterior_samples.npy")
    ...
    results_path = os.path.join(args.outdir, "sbi_2dps_wedge_filtered_results.npz")
    np.savez(
        results_path,
        samples=samples_np,
        ...
        wedge_C=np.array(args.wedge_C, dtype=np.float32),
        wedge_keep_fraction=np.array(float(np.mean(wedge_mask)), dtype=np.float32),
        summary_mode="wedge_filtered_noisy_ms_2dps_no_noise_mean_subtraction",
    )
    ...
    corner_path = os.path.join(args.outdir, "snpe_2dps_wedge_filtered_corner.png")
    plt.savefig(corner_path, dpi=300, bbox_inches="tight")
    plt.close()
    logger.info("Saved corner plot: %s", corner_path)

    logger.info("PARALLEL WEDGE-FILTERED NOISY 2DPS SBI COMPLETED SUCCESSFULLY")
